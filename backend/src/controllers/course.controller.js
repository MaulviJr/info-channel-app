...
const listCourses = asyncHandler(async (req, res) => {
    ...
    const courses = await courseService.getPublishedCourses(Number(page), Number(limit));
...
const listAllCoursesForStaff = asyncHandler(async (req, res) => {
    ...
    const courses = await courseService.getStaffCourses(req.user, Number(page), Number(limit));
...
const toggleCoursePublish = asyncHandler(async (req, res) => {
    ...
    const newStatus = await courseService.toggleCoursePublish(id, req.user);
    
    return res.status(200).json(
        new ApiResponse(200, { is_published: newStatus }, `Course ${newStatus ? 'published' : 'unpublished'} successfully`)
    );
});
